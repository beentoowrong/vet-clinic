...
export class UsersService {
  ...
  async updateMe(userId: number, updateMeDto: UpdateMeDto) {
    this.logger.debug(
      `UserService.update( ${JSON.stringify(userId)}, ${JSON.stringify(updateMeDto)} )`,
    );
  ...
  // change credential user by admin
  async changeCredential(currentUser: ActiveUserData, userId: number, updateCredentialDto: UpdateCredentialDto) {
    this.logger.debug(
      `UserService.changeCredential( ${JSON.stringify(userId)}, ${JSON.stringify(updateCredentialDto)} )`
    )
    ...
    if (email !== undefined || role !== undefined) {
      if (currentUser.role !== Role.ADMIN && currentUser.role !== Role.SUPER_ADMIN) {
        throw new ForbiddenException (
          'Only Admin or Super Admin can update email'
        )
      }
    ...
    return {
      status: 200,
      message: 'User credential has been succesfully updated',
      data: updateCredentialUser
    }
  }
  ...
  async changePassword(userId: number, changePasswordDto: ChangePasswordDto) {
    this.logger.debug(
      `UserService.changePassword( ${JSON.stringify(userId)}, ${JSON.stringify(changePasswordDto)} )`,
    );

    // Validasi konfirmasi password baru
    if (
      changePasswordDto.new_password !==
      changePasswordDto.new_password_confirmation
    ) {
  ...
  async createUser(
    ...
    this.logger.debug(
      `UserService.createUser( ${JSON.stringify(currentUser)}, ${JSON.stringify(createUserDto)} )`,
    );
  ...
  async findAllPaginatedUser(paginationDto: PaginationDto) {
